        ...
        {!sent && (
          <motion.div key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }} className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-10">
            <div className="text-center mb-8">
              ...
              <p className="text-slate-500 text-sm mt-2 leading-relaxed">Informe seu e-mail e enviaremos um link para redefinir sua senha.</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" className="pl-10 py-6 rounded-2xl" required />
                </div>
              </div>
              <Button type="submit" className="w-full py-6 bg-[#7E22CE] hover:bg-purple-700 text-white rounded-2xl font-bold text-base" disabled={loading}>
                {loading ? (<span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Enviando...</span>) : "Enviar link de recuperacao"}
              </Button>
            </form>
            <button onClick={() => navigate("/auth")} className="w-full mt-4 flex items-center justify-center gap-2 text-sm text-slate-500 hover:text-[#7E22CE] transition-colors">
              <ArrowLeft className="w-4 h-4" /> Voltar ao login
            </button>
          </motion.div>
        )}
        {sent && (
          <motion.div key="sent" initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-10 text-center">
            ...
            <p className="text-slate-500 text-sm leading-relaxed mb-6">
              Enviamos um link de recuperacao para<br />
            ...
            <div className="bg-purple-50 border border-purple-100 rounded-2xl p-4 mb-6 text-left">
              ...
              <p className="text-sm text-purple-600 mt-1">Nao recebeu? Aguarde alguns minutos e tente novamente.</p>
            </div>
