        ...
        <div className="overflow-y-auto max-h-[360px]">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-[#7E22CE] text-white z-10">
              <tr>
                <th className="py-2.5 px-3 text-center font-semibold">Nº</th>
                <th className="py-2.5 px-3 text-right font-semibold">Parcela</th>
                <th className="py-2.5 px-3 text-right font-semibold">Amortização</th>
                <th className="py-2.5 px-3 text-right font-semibold">Juros</th>
                <th className="py-2.5 px-3 text-right font-semibold">Taxas</th>
...
type Simulacao = {
  ...
  rendaSuficiente: boolean; // novo campo para indicar se a renda atende à parcela
};
...
const BancoResultCard = ({ banco, melhorSimulacaoId }: { banco: BancoAgrupado; melhorSimulacaoId: string | null }) => {
  const [expanded, setExpanded] = useState<string | null>(null); // 'price' | 'sac' ou null
...
const FinanciamentoCalc = () => {
  ...
  const calcular = () => {
    ...
    const taxasMensais = parseFloat(form.taxasMensais.replace(/\D/g, "")) / 100 || 0;
    ...
    for (const banco of bancos.filter(b => bancosAtivos.includes(b.id))) {
      ...
      if (entradaMin > entradaMinimaNecessaria) {
        entradaMinimaNecessaria = entradaMin;
      }
    }
    ...
    if (dataNascimento && prazoMaxUser < prazoSolicitado) {
      ...
      setPrazoMaximoAviso(`⚠️ O prazo foi ajustado individualmente em alguns bancos devido à idade do proponente (${idade} anos).`);
    } else {
      setPrazoMaximoAviso("");
    }
    ...
    for (const banco of bancos) {
      ...
      for (const sis of sistemas) {
        ...
        if (simuladoPor === "financiamento") {
          ...
          const parcelaBase = parcelaDesejada - taxasMensais;
          ...
          const parcelaMax = rendaMensal * 0.3 - taxasMensais;
          // mesmo que parcelaMax seja negativo, calculamos (pode ser útil para debug)
          if (sis === "price") {
        ...
        // Verificar se a entrada calculada atende ao mínimo do banco (para todos os modos)
        const percentMax = banco.maxFinancingPercent[tipoImovel][tipoUso];
        ...
        // Verificar se a renda é suficiente (apenas para o modo renda)
        ...
        const simulacao: Simulacao = {
          ...
          ultimaParcela,
          totalJuros,
          totalPago: financiadoCalc + totalJuros + taxasMensais * prazoEfetivo,
    ...
    if (!algumaSimulacaoGerada) {
      if (simuladoPor === "renda") {
        setNenhumaSimulacaoMsg("Nenhum banco atende à sua renda com a entrada mínima exigida. Tente aumentar a renda ou reduzir o valor do imóvel.");
      } else if (simuladoPor === "parcela") {
        setNenhumaSimulacaoMsg("Nenhum banco atende à parcela desejada com a entrada mínima exigida. Tente aumentar a parcela ou reduzir o valor do imóvel.");
      } else {
    ...
    // Para exibir os resumos, usamos a primeira simulação (a melhor)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input 
            ...
            placeholder="420" 
          />
          <p className="text-xs text-muted-foreground">{Math.round(parseInt(form.prazo || "0") / 12)} anos (máx 420 meses)</p>
        </div>
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Valida o limite da idade de 80 anos e 6m</p>
        </div>
      </div>
      ...
      {/* Mensagem quando nenhuma simulação é encontrada */}
      {nenhumaSimulacaoMsg && (
        <Alert className="bg-amber-50 border-amber-200">
          <AlertCircle className="h-4 w-4 text-amber-600" />
...
const ComissaoCalc = () => {
  ...
  const handleMoney = (val: string) => {
    const nums = val.replace(/\D/g, "");
    setForm(f => ({ ...f, valorVenda: nums }));
  };

  const displayMoney = (val: string) => {
    if (!val) return "";
    return (parseInt(val) / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
  };
...
const RentabilidadeCalc = () => {
  ...
  const handleMoney = (field: string, val: string) => {
    const nums = val.replace(/\D/g, "");
    setForm(f => ({ ...f, [field]: nums }));
  };
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <p className="text-sm text-blue-800 font-medium mb-2">⚖️ Comparativo de Mercado (Estimativa em 1 ano):</p>
