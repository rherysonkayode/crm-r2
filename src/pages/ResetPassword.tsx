...
const ResetPassword = () => {
  ...
  useEffect(() => {
    // URL formato: /#/reset-password#access_token=...&type=recovery
    // Tem dois # entao pegamos tudo apos o segundo #
    const href = window.location.href;
    const firstHash = href.indexOf("#");
    const secondHash = firstHash !== -1 ? href.indexOf("#", firstHash + 1) : -1;
    const tokenString = secondHash !== -1 ? href.substring(secondHash + 1) : "";
  ...
  const strengthLabel = ["", "Muito fraca", "Fraca", "Media", "Forte", "Muito forte"][passwordStrength];

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isPasswordValid) { toast.error("A senha nao atende aos requisitos minimos"); return; }
      ...
      <AnimatePresence mode="wait">
        ...
        {sessionError && !success && (
          <motion.div key="error" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-10 text-center">
            ...
            <h1 className="text-2xl font-black text-slate-900 mb-2">Link invalido</h1>
            <p className="text-slate-500 text-sm mb-6">Este link expirou ou ja foi utilizado. Solicite um novo link de recuperacao.</p>
          ...
          <motion.div key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }} className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                {confirmPassword && password !== confirmPassword && <p className="text-xs text-red-500">As senhas nao coincidem</p>}
