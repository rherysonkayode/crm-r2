              ...
              <DialogContent className="sm:max-w-lg">
                ...
                <div className="grid gap-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Anotacoes sobre o lead..." />
                  </div>
          ...
          <div>
            {(!kanbanColumns || kanbanColumns.length === 0) ? (
              <div className="flex flex-col items-center justify-center py-20 bg-card border border-dashed border-border rounded-xl text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">Clique no icone de configuracoes para criar suas colunas</p>
                ...
                <div className="space-y-3 bg-slate-50 rounded-xl p-4">
                  {viewingLead.phone && (
                    <div className="flex items-center gap-3">
                      <Phone className="w-4 h-4 text-[#7E22CE] shrink-0" />
                      <a href={`tel:${viewingLead.phone}`} className="text-sm text-slate-700 hover:text-[#7E22CE] transition-colors">{viewingLead.phone}</a>
                    </div>
                  )}
                  {viewingLead.email && (
                    <div className="flex items-center gap-3">
                      <Mail className="w-4 h-4 text-[#7E22CE] shrink-0" />
                      <a href={`mailto:${viewingLead.email}`} className="text-sm text-slate-700 hover:text-[#7E22CE] transition-colors truncate">{viewingLead.email}</a>
                    </div>
                  )}
                  {viewingLead.cpf && (
                    <div className="flex items-center gap-3">
                  ...
                  {isImobiliaria && (
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-slate-700">
                        {profiles?.find(p => p.id === viewingLead.assigned_to)?.full_name || "Nao atribuido"}
                      </span>
                    </div>
                  )}
                </div>
